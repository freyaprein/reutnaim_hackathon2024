...
def plot_participant_data(file_1, file_2, file_3, label_1, label_2, label_3):
   ...
   #data4 = pd.read_csv(Acceleration)
    ...
    for tag in tags:
        ax1.axvline(x=tag, color='black', linestyle='--', label=f'Tag {tag}')
        ax2.axvline(x=tag, color='black', linestyle='--', label=f'Tag {tag}')
        ax3.axvline(x=tag, color='black', linestyle='--', label=f'Tag {tag}')
        ymax = ax1.get_ylim()[1]
        # Add text at the top of the vertical line
        ax1.text(x=tag, y=ymax, s=str(tag), color='black', ha='center', va='bottom', fontsize=10)


    # For ax3, ensure that each tag is only labeled once in the legend
    ...
    ax3.set_xlim(min(time1.min(), time2.min()), max(time1.max(), time2.max()))
