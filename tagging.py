...
def read_data_files(folder):
    """Reads the HR, BVP, TEMP, EDA, and tags files from the specified folder."""
    ...
    return hr_data, bvp_data, temp_data, eda_data, tags_data

def calculate_differences(data, tag_A1, tag_A2, tag_A3):
    """Calculates the differences between the tag values and the data."""
    A1 = data.iloc[0, 0]
    A2 = data.iloc[1, 0]
...
def update_time_column(data, diff1, diff2, diff3):
    """Updates the time column for the data based on the calculated differences."""
    time_column = [0] * len(data)
...
def process_folder(folder):
    """Processes the data in a specific folder and saves the modified files."""
    hr_data, bvp_data, temp_data, eda_data, tags_data = read_data_files(folder)

    # Extract the necessary values from tags.csv
    tag_A1, tag_A2, tag_A3 = tags_data.iloc[0, 0], tags_data.iloc[1, 0], tags_data.iloc[2, 0]

    # Calculate differences for HR, BVP, TEMP, and EDA
    hr_diff1, hr_diff2, hr_diff3 = calculate_differences(hr_data, tag_A1, tag_A2, tag_A3)
    bvp_diff1, bvp_diff2, bvp_diff3 = calculate_differences(bvp_data, tag_A1, tag_A2, tag_A3)
    temp_diff1, temp_diff2, temp_diff3 = calculate_differences(temp_data, tag_A1, tag_A2, tag_A3)
    eda_diff1, eda_diff2, eda_diff3 = calculate_differences(eda_data, tag_A1, tag_A2, tag_A3)
    ...
    # Update the time columns for each data set
    hr_data.insert(0, 'time(s)', update_time_column(hr_data, hr_diff1, hr_diff2, hr_diff3))
    bvp_data.insert(0, 'time(s)', update_time_column(bvp_data, bvp_diff1, bvp_diff2, bvp_diff3))
    temp_data.insert(0, 'time(s)', update_time_column(temp_data, temp_diff1, temp_diff2, temp_diff3))
    eda_data.insert(0, 'time(s)', update_time_column(eda_data, eda_diff1, eda_diff2, eda_diff3))
...
# Specify the base path and process all files
